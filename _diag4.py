import sqlite3
conn = sqlite3.connect('database/accounting_dev.db')
cur = conn.cursor()

# أعمدة جدول businesses
cur.execute('PRAGMA table_info(businesses)')
biz_cols = [c[1] for c in cur.fetchall()]
print('أعمدة businesses:', biz_cols)

# فحص البيزنيسات
cur.execute('SELECT id, name FROM businesses ORDER BY id')
biznses = cur.fetchall()
print('\nالبيزنيسات:')
for b in biznses[:15]:
    cur.execute('SELECT COUNT(*) FROM products WHERE business_id=?', (b[0],))
    cnt = cur.fetchone()[0]
    print(f'  [{b[0]}] {b[1]} | منتجات={cnt}')

# أعمدة settings
cur.execute('PRAGMA table_info(settings)')
settings_cols = [c[1] for c in cur.fetchall()]
print('\nأعمدة settings:', settings_cols)

cur.execute('SELECT * FROM settings WHERE business_id=1 LIMIT 5')
rows = cur.fetchall()
if rows:
    for r in rows:
        print(' ', r)
else:
    print('لا يوجد إعدادات لـ business 1')

# business_settings_ext
cur.execute('PRAGMA table_info(business_settings_ext)')
ext_cols = [c[1] for c in cur.fetchall()]
print('\nأعمدة business_settings_ext:', ext_cols)

cur.execute('SELECT key, value FROM business_settings_ext WHERE business_id=1 LIMIT 15')
for r in cur.fetchall():
    print(' ', r)

conn.close()
